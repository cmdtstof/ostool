...
$sql = "select * from (select project,DATE_FORMAT(event_time,'".$datetimeContion."'),source,event_name,count(1) as c from event_record where project = '".$project_code."' group by project,DATE_FORMAT(event_time,'".$datetimeContion."'),source,event_name)A  order by A.c desc ";
...
if(mysql_num_rows($result)>0){
	...
	$showmsg = $showmsg."<thead><tr><td>项目</td><td>时间</td><td>来源</td><td>事件</td><td>访问数</td></tr></thead>";
	$showmsg= $showmsg."<tbody>";
	while ($row=mysql_fetch_row($result))
	{
		$showmsg= $showmsg."<tr><td>".$row[0]."</td><td>".$row[1]."</td><td>".$row[2]."</td><td>".$row[3]."</td><td>".$row[4]."</td></tr>";
